...
describe('IO', () => {
  ...
  const filePath: string = `./dist/src/data/${fileName}.json`;

  describe('Save to JSON', () => {
    saveToJSON(fileName, obj);

    it('Should create a new directory', () =>
      expect(dir('./dist/src/data')).to.exist);

    it('Should create a new file', () => expect(file(filePath)).to.exist);

    it('Should match content', () =>
      expect(file(filePath)).to.equal('{"str":"value","num":1}'));
  });
    ...
    describe('Private car', () => {
      it('Should return an array', async () => {
        ...
        if (res.length > 0) {
          ...
          expect(res[0])
            .to.have.property('park_Id')
            .that.is.a('string');
          expect(res[0])
            .to.have.property('privateCar')
            .that.is.an('array');
        }
      });
    });
  ...
  describe('Build Response', () => {
    describe('Item', () => {
      ...
      expect(item.image)
        ...
        .that.is.a('string');
      expect(item.image)
        .to.have.property('accessibilityText')
        .that.is.a('string');
    });
  });
});
